    ...
    this.decrire = function () {
      ...
      //monHero.attaquer(monstre)
        this.attaquer = function (cible) {
            if(cible.sante > 0) { // si la cible a encore de la santé
                var degats = this.force;
                document.write(this.pseudo + " attaque " + cible.pseudo + " et lui fait " + degats + " points de dégâts" + '<br><br>');
                cible.sante= cible.sante - degats; // réduit la santé de la cible.
                if(cible.sante <= 0) {  // si santé inf à 0
                    cible.sante = 0; // pour éviter le négatif
                    document.write(cible.pseudo + " est mort !!<br><br>"); 
                    this.xp += 100*50/100;  // il prend 50% d'expérience
                    if(this.xp >= level_up) { // si l'exp est sup au level up définit à 100 
                        this.level += 1; // il prend un niveau
                        document.write("Level Up !<br><br>");
                        this.xp = 0;
                        level_up += level_up*25/100; // exp + 25%
                    }
                }
                document.write(cible.pseudo + " lui reste " + cible.sante + " points de vies<br><br>"); // s'il lui reste de la santé on lui indique
            }
        },
        this.soigner = function(cible){  // fonction soigner 
            if(this.classe == 'Magicien'){  // le magicien peut soigner de 50% le personnage en question
